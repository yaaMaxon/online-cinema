...
import { useGetMovieCastQuery } from "@/app/store/api/features/movieApi";
...
const MovieCast = () => {
  ...
  const handleNext = () => {
    if ((currentPage + 1) * itemsPerPage < movieCast?.cast.length) {
  ...
  const displayedCast = movieCast?.cast.slice(
      ...
      <div className="flex justify-between items-center mb-5">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            onClick={handleNext}
            disabled={
              (currentPage + 1) * itemsPerPage >= movieCast?.cast.length
            }
          >
            <div className="bg-[#141414] border-[1px] border-[#262626] rounded-full p-3">
              <ArrowRightIcon />
        ...
        {displayedCast?.map(({ id, profile_path }: Cast) => (
          <li key={id}>
            <Image
              ...
              width={87}
              height={89}
              className="rounded-[10px] w-[70px] h-[85px] lg:w-[95px] lg:h-[100px]"
            />
          </li>
        ))}
      </ul>
