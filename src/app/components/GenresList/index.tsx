...
const GenresList = ({ title, label, titleStyles }: Props) => {
  ...
  const [currentPage, setCurrentPage] = useState(0);
  const [swiperIndex, setSwiperIndex] = useState(0); // Задаємо змінну для індексу слайда в Swiper
  const itemsPerPage = 5;
  const totalPages = Math.ceil(genreMovies?.genres.length / itemsPerPage);
  ...
  const progressPercentage = ((currentPage + 1) / totalPages) * 100;
  const progressPercentageForModile = ((swiperIndex + 1) / totalSlides) * 100; // Обчислюємо прогрес за індексом слайда
  ...
  return (
    <>
      <div
        id="genres"
        className="flex lg:justify-between lg:items-center mb-5 lg:mb-[60px] scroll-mt-32"
      >
        {(title || label) && (
          <div>
            {title && (
